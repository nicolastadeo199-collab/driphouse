...
export default function CategoryTiles({ categories }: { categories: CategoryTile[] }) {
  ...
  return (
    <div className="mb-8 flex gap-3 overflow-x-auto pb-1 scrollbar-thin">
      {categories.map((category) => (
        <Link
          key={category.id}
          href={`/?categoria=${category.slug}`}
          className="group relative block h-40 w-32 shrink-0 overflow-hidden rounded-lg border border-border bg-black sm:h-48 sm:w-40"
        >
          {category.image ? (
            <Image
              src={category.image}
